...
class Settings_LoginHistory_Record_Model extends Settings_Vtiger_Record_Model {
	...
	public function getDisplayValue($fieldName, $recordId = false) {
		...
		} else if ($fieldName == 'session_duration') {
			if ($fieldValue === null || $fieldValue === '' || $this->get('status') == 'Signed in') {
				$fieldValue = '---';
			} else {
				$seconds = intval($fieldValue);
				$hours = floor($seconds / 3600);
				$minutes = floor(($seconds % 3600) / 60);
				$secs = $seconds % 60;
				
				if ($hours > 0) {
					$fieldValue = sprintf('%d giờ %d phút', $hours, $minutes);
				} else if ($minutes > 0) {
					$fieldValue = sprintf('%d phút %d giây', $minutes, $secs);
				} else {
					$fieldValue = sprintf('%d giây', $secs);
				}
			}
		}
		return $fieldValue;
